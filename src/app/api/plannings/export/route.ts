...
import { getPublished, type PlanningRow } from '@/lib/plannings';
...
function toCsv(planning: PlanningRow, doctors: string[]): string {
  const header = ['Médecin', ...planning.days.map((d) => `${WEEKDAYS_FR[d.weekday]} ${d.day}`)].join(';');
...
async function toXlsx(planning: PlanningRow, doctors: string[], year: number, month: number): Promise<Uint8Array> {
